        ...
        for n in ren:
            if (n==";"):
                ...
                if (p>1):
                    z=False
                    l.append(d)
    return z,l,"Se encontro mas de un ; en la linea"
...
if (coma[0]==True):
    ...
    for ren in archivo:
        ...
        if (datos != "end;"):
            ...
            dt=codigop1(tok)
            if dt!="":
                primeraparte.append(dt)
            if(tok[0] == "var"):#2
                agregaVar(datos,contador)
                contador+=1
            elif (tok[1]=="="):#4 
                if len(tok)==4:
                    setValor(tok[0],tok[2])
                else:
                    d= ren.split("=") #5
                    expresion=d[1][:-2]
    ...
    print(f"Error falta ; en la linea {coma[1]} o\n{coma[2]} {coma[1]}")
archivo.close()
...
for c in primeraparte:
    print(c)
for c in terceraparte:
    for v in tablaVar:
        p=c.split()
        if (v.nombre==p[1][:-1]):
            g=str(v.direccion)
